# ============================================================================
# PolyForm — demonstration occupancy metrics on real observed supports
#
# Purpose:
#   The human proteoform catalogue provides proteoform identities, but not valid
#   quantitative occupancy weights, because MS1 abundance is not an injective map
#   to MS2-resolved proteoform identity.
#
#   Therefore, this script does NOT infer biological occupancy from the catalogue.
#   Instead, it selects real proteins with broad observed k-range access and then
#   assigns explicit demonstration weights over the observed binary support.
#
#   This shows how PolyForm computes occupancy-dependent metrics when a valid
#   weight vector is supplied.
#
# Outputs:
#   SELECTED_PROTEINS        = proteins selected for demonstration
#   DEMO_STATE_WEIGHTS      = state-level demonstration weights
#   DEMO_METRICS            = protein/profile-level occupancy metrics
#   DEMO_GRADE              = grade-level occupancy distributions
#   DEMO_COMPONENTS         = native support components and component weights
#
# Figures:
#   polyform_02_demo_occupancy_metrics.png/pdf
#   polyform_02_focus_state_weight_heatmap.png/pdf
# ============================================================================
...
# If your repo is cloned in Colab, set this to the repo root.
# Example: REPO_DIR = "/content/PolyForm"
REPO_DIR = "/content/PolyForm"

OUTDIR = os.path.join(REPO_DIR, "manuscript", "polyform_02_demo_occupancy_outputs")
if not os.path.exists(REPO_DIR):
    OUTDIR = "polyform_02_demo_occupancy_outputs"

os.makedirs(OUTDIR, exist_ok=True)

SELECT_N = 8                     # number of proteins selected for demonstration
MIN_STATES = 5                   # require at least this many accessed states if possible
MIN_DELTA_K = 3                  # require at least this grade span if possible
DPI = 300
RANDOM_SEED = 7
...
fig.savefig(os.path.join(OUTDIR, "polyform_02_demo_occupancy_metrics.png"), dpi=DPI)
fig.savefig(os.path.join(OUTDIR, "polyform_02_demo_occupancy_metrics.pdf"))
plt.show()
...
fig.savefig(os.path.join(OUTDIR, "polyform_02_focus_state_weight_heatmap.png"), dpi=DPI)
fig.savefig(os.path.join(OUTDIR, "polyform_02_focus_state_weight_heatmap.pdf"))
plt.show()

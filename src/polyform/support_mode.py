...
def run_support_mode(csv="proteoforms.csv", fasta_path=None, pos_base=0,
                     outdir=".", make_figures=True, dpi=300):
    """Global support-mode structural metrics on the choice-free modal lattice.

    Reads a proteoform catalogue CSV and computes support/access metrics,
    grade distributions and fibre refinement. Returns a dict of DataFrames
    and (by default) writes CSVs + a 9-panel figure into `outdir`."""
    ...
    GLOBAL.to_csv(os.path.join(outdir, "polyform_global_summary.csv"), index=False)
    METS_EXPORT.to_csv(os.path.join(outdir, "polyform_protein_metrics.csv"), index=False)
    GRADE.to_csv(os.path.join(outdir, "polyform_grade_metrics.csv"), index=False)
    GRADE_GLOBAL.to_csv(os.path.join(outdir, "polyform_global_grade_metrics.csv"), index=False)
    RESIDUES.to_csv(os.path.join(outdir, "polyform_residue_counts.csv"), index=False)
    CODE_EXPORT.to_csv(os.path.join(outdir, "polyform_resid_code_residue_map.csv"), index=False)
    TOP_ACCESS.to_csv(os.path.join(outdir, "polyform_top_accessed_states.csv"), index=False)
    TOP_SPAN.to_csv(os.path.join(outdir, "polyform_top_grade_span.csv"), index=False)
    TOP_FIBRE.to_csv(os.path.join(outdir, "polyform_top_fibre_refinement.csv"), index=False)
    ...
    if make_figures: fig.savefig(os.path.join(outdir, "polyform_global_structural_metrics.png"), dpi=DPI)
    if make_figures: fig.savefig(os.path.join(outdir, "polyform_global_structural_metrics.pdf"))
    pass
    ...
    return {
        "global": GLOBAL,
        "protein_metrics": METS,
        "grade": GRADE,
        "grade_global": GRADE_GLOBAL,
        "residues": RESIDUES,
        "code_residue": CODE_RESIDUE,
